...
import { Workflow, Trash2, Plus, X, FileText, Info, Tag, Search, Filter, GitBranch } from 'lucide-react';
import { createWorkflowStep, createStepOption, addNestedComponent, removeNestedComponent } from '../../types/template.types.js';
...
export const WorkflowStepsEditor = ({ 
      ...
      if (step.id === stepId) {
        return {
          ...
          options: (step.options || []).map(opt => 
            opt.id === optionId 
              ? { ...opt, nestedComponent: component, componentId: null }
              : opt
          )
        };
      }
    ...
    return (
      <div className="mt-4 p-4 bg-secondary-50 dark:bg-secondary-900/30 rounded-lg border border-secondary-200 dark:border-secondary-700">
        <div className="flex items-center justify-between mb-3">
          <h5 className="font-medium text-secondary-800 dark:text-secondary-200">Select {type.charAt(0).toUpperCase() + type.slice(1)}</h5>
          <button
        ...
        <SearchAndFilter
          ...
          renderItem={(item) => (
            <div>
              <div className="font-medium text-secondary-900 dark:text-secondary-100">{item.name}</div>
              {item.description && (
                <div className="text-sm text-secondary-600 dark:text-secondary-400 mt-1">{item.description}</div>
              )}
              {type === 'snippet' && item.tags && (
                <div className="flex flex-wrap gap-1 mt-2">
                  {item.tags.map(tag => (
                    <span key={tag} className="px-2 py-1 bg-secondary-200 dark:bg-secondary-700 text-secondary-700 dark:text-secondary-300 rounded text-xs">
                      {tag}
                    </span>
                  ))}
                </div>
      ...
      <div className="mt-4 space-y-4">
          ...
          <div className={`p-4 rounded-lg ${getColorClasses('template', 'info-block')}`}>
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h6 className={`font-medium ${getColorClasses('template', 'info-header')}`}>Selected Template</h6>
                  <p className={`text-sm ${getColorClasses('template', 'info-text')}`}>{selectedTemplate.name}</p>
                </div>
              </div>
              <button
                onClick={() => handleRemoveGlobalComponent(step.id, 'template', selectedTemplate.id)}
        ...
        {nestedTemplates.length > 0 && (
          <div className="p-4 bg-primary-50 dark:bg-primary-900/30 rounded-lg border border-primary-200 dark:border-primary-700">
            <h6 className="font-medium text-primary-800 dark:text-primary-200 mb-2">Local Template</h6>
            {nestedTemplates.map(template => (
              <div key={template.id} className="flex items-center justify-between">
                <span className="text-sm text-primary-700 dark:text-primary-300">{template.name}</span>
                <button
          ...
          <div className="border-2 border-dashed border-secondary-300 dark:border-secondary-600 rounded-lg p-6 text-center">
            ...
            <div className="flex justify-center gap-3">
              ...
              <button
                ...
                className="px-4 py-2 border border-primary-600 text-primary-600 rounded hover:bg-primary-50 dark:hover:bg-primary-900/50 transition-all duration-200"
              >
                Create New
              </button>
            </div>
        ...
        {showGlobalSelector && showGlobalSelector.stepId === step.id && showGlobalSelector.type === 'template' && (
      ...
      <div className="mt-4 space-y-4">
        ...
        {selectedSnippets.length > 0 && (
          <div className={`p-4 rounded-lg ${getColorClasses('snippet', 'info-block')}`}>
            <h6 className={`font-medium mb-3 ${getColorClasses('snippet', 'info-header')}`}>Selected Snippets</h6>
            <div className="flex flex-wrap gap-2">
              {selectedSnippets.map(snippet => (
                <div key={snippet.id} className={`flex items-center gap-2 px-3 py-1 rounded text-sm ${getColorClasses('snippet', 'button')}`}>
        ...
        {nestedSnippets.length > 0 && (
          <div className="p-4 bg-success-50 dark:bg-success-900/30 rounded-lg border border-success-200 dark:border-success-700">
            <h6 className="font-medium text-success-800 dark:text-success-200 mb-3">Local Snippets</h6>
            <div className="flex flex-wrap gap-2">
              {nestedSnippets.map(snippet => (
                <div key={snippet.id} className="flex items-center gap-2 px-3 py-1 bg-success-100 dark:bg-success-900/50 text-success-800 dark:text-success-200 rounded text-sm">
                  <Tag className="w-3 h-3" />
                  <span>{snippet.name}</span>
                  <button
        ...
        <div className="border border-secondary-300 dark:border-secondary-600 rounded-lg p-4">
          <div className="flex items-center justify-between mb-3">
            ...
            <span className="text-xs text-secondary-500 dark:text-secondary-400">Multiple selections allowed</span>
          </div>
          <div className="flex gap-3">
            <button
              type="button"
              onClick={() => showAddGlobalComponent(step.id, 'snippet')}
              className="flex-1 px-4 py-2 bg-green-600 text-white rounded hover:bg-green-700 transition-all duration-200"
            >
            ...
            <button
              ...
              className="flex-1 px-4 py-2 border border-success-600 text-success-600 rounded hover:bg-success-50 dark:hover:bg-success-900/50 transition-all duration-200"
            >
              <Plus className="w-4 h-4 inline mr-2" />
              Create New
            </button>
        ...
        {showGlobalSelector && showGlobalSelector.stepId === step.id && showGlobalSelector.type === 'snippet' && (
          renderGlobalComponentSelector(step)
        )}
      </div>
    );
      ...
      <div className="mt-4 space-y-4">
          ...
          <div className={`p-4 rounded-lg ${getColorClasses('workflow', 'info-block')}`}>
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h6 className={`font-medium ${getColorClasses('workflow', 'info-header')}`}>Selected Workflow</h6>
                  <p className={`text-sm ${getColorClasses('workflow', 'info-text')}`}>{selectedWorkflow.name}</p>
                </div>
              </div>
              <button
                onClick={() => handleRemoveGlobalComponent(step.id, 'workflow', selectedWorkflow.id)}
        ...
        {nestedWorkflows.length > 0 && (
          <div className="p-4 bg-secondary-50 dark:bg-secondary-900/30 rounded-lg border border-secondary-200 dark:border-secondary-700">
            <h6 className="font-medium text-secondary-800 dark:text-secondary-200 mb-2">Local Workflow</h6>
            {nestedWorkflows.map(workflow => (
              <div key={workflow.id} className="flex items-center justify-between">
                <span className="text-sm text-secondary-700 dark:text-secondary-300">{workflow.name}</span>
                <button
        ...
        {!selectedWorkflow && nestedWorkflows.length === 0 && (
          <div className="mt-4 p-4 bg-secondary-50 dark:bg-secondary-900/30 rounded-lg border border-secondary-200 dark:border-secondary-700">
            <div className="flex items-center justify-between mb-3">
              <h5 className="font-medium text-secondary-800 dark:text-secondary-200">Select Workflow</h5>
            </div>
      ...
      <div className="mt-4 pt-4 border-t border-secondary-200 dark:border-secondary-600">
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-2">
            ...
            <h5 className="font-medium text-secondary-800 dark:text-secondary-200">Choice Options</h5>
          </div>
          <button
            type="button"
            onClick={() => addOptionToStep(step.id)}
        ...
        <div className="space-y-3">
          {step.options.map((option) => (
            <div key={option.id} className="p-4 bg-secondary-50 dark:bg-secondary-900/20 rounded-lg border border-secondary-200 dark:border-secondary-700">
              <div className="flex items-center justify-between mb-3">
                <input
                  type="text"
                  value={option.name}
              ...
              <div className="space-y-2">
                <label className="text-sm font-medium text-secondary-700 dark:text-secondary-300">Select {step.type} for this option:</label>
  ...
  const renderOptionComponentSelector = (step, option) => {
    ...
    if (!option.type) {
      return (
        <div className="space-y-3">
          <p className="text-sm font-medium text-secondary-700 dark:text-secondary-300">Choose type for this option:</p>
          <div className="flex justify-center gap-3">
            <button
              type="button"
        ...
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm font-medium text-secondary-700 dark:text-secondary-300">Info Step</span>
            </div>
            <button
              onClick={() => {
          ...
          <p className="text-xs text-secondary-600 dark:text-secondary-400">Info steps don't require a component selection.</p>
        </div>
      );
    }
    
    // Get components based on option type
    const components = option.type === 'template' ? templates : 
                      option.type === 'snippet' ? snippets : 
                      option.type === 'workflow' ? workflows : [];
    
    const selectedComponent = option.componentId ? 
      components.find(c => c.id === option.componentId) : null;
        ...
        <div className="border-2 border-dashed border-secondary-300 dark:border-secondary-600 rounded-lg p-4">
          ...
          {option.type === 'workflow' ? (
            <SearchAndFilter
              ...
              onSearchChange={(term) => setSearchTerm(step.id, `choice-${option.id}-workflow`, term)}
              filterFolder={getFilterFolder(step.id, `choice-${option.id}-workflow`)}
              onFilterChange={(folder) => setFilterFolder(step.id, `choice-${option.id}-workflow`, folder)}
              folders={folders}
            />
          ) : (
            /* Direct SearchAndFilter for template and snippet types */
            <>
              {showChoiceOptionLibrary?.stepId === step.id && showChoiceOptionLibrary?.optionId === option.id ? (
                <div>
                  <SearchAndFilter
                    type={option.type}
                    items={components}
                    ...
                    onSearchChange={(term) => setSearchTerm(step.id, `choice-${option.id}-${option.type}`, term)}
                    filterFolder={getFilterFolder(step.id, `choice-${option.id}-${option.type}`)}
                    onFilterChange={(folder) => setFilterFolder(step.id, `choice-${option.id}-${option.type}`, folder)}
                    folders={folders}
                  />
                ...
                <div className="flex justify-center gap-3">
                  <button
                    type="button"
                    onClick={() => setShowChoiceOptionLibrary({ stepId: step.id, optionId: option.id })}
                    className="px-4 py-2 bg-blue-600 text-white rounded hover:bg-blue-700 transition-all duration-200"
                  >
                    Select from Library
                  </button>
    ...
    <>
        ...
        {steps.length === 0 ? (
          ...
          <div className="space-y-6">
            {steps.map((step, index) => (
              <div key={step.id} className="bg-white dark:bg-secondary-800 border border-secondary-200 dark:border-secondary-700 rounded-lg p-6">
                {/* Step Header */}
      ...
      {showNestedCreator && (
        <NestedComponentCreator
          ...
          onCreateComponent={(type, component) => handleCreateNestedComponent(showNestedCreator.stepId, type, component)}
      ...
      {showChoiceOptionCreator && (
        <NestedComponentCreator
          ...
          onCreateComponent={(type, component) => handleCreateChoiceOptionComponent(showChoiceOptionCreator.stepId, showChoiceOptionCreator.optionId, type, component)}
