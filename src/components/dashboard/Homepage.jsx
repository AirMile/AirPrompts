...
const Homepage = ({
  ...
  const handleUpdateFolderDescription = useCallback(
  ...
  useEffect(() => {
    if (folders && folders.length > 0 && selectedFolderId) {
      ...
      const folderExists =
        folders.some((f) => f.id === selectedFolderId) || selectedFolderId === 'root';
      if (!folderExists) {
        console.warn(
          `Selected folder '${selectedFolderId}' does not exist, falling back to 'root'`
        );
      ...
      const folderFiltered = items.filter((item) => {
        ...
        if (!selectedFolderId || selectedFolderId === 'root') {
          ...
          if (item.folderIds && item.folderIds.length > 0) {
            folderMatch = item.folderIds.some(
              (id) =>
                !id ||
                id === 'root' ||
                (id !== 'projects' &&
                  id !== 'ai-character-story' &&
                  id !== 'prompt-website' &&
                  id !== 'rogue-lite-game' &&
                  id !== 'content')
            );
            ...
            folderMatch =
              !item.folderId ||
              item.folderId === 'root' ||
              (item.folderId !== 'projects' &&
                item.folderId !== 'ai-character-story' &&
                item.folderId !== 'prompt-website' &&
                item.folderId !== 'rogue-lite-game' &&
  ...
  const filteredTemplates = useMemo(
    () => getFilteredItems(templates, 'template'),
    [templates, getFilteredItems]
  );
  const filteredWorkflows = useMemo(
    () => getFilteredItems(workflows, 'workflow'),
    [workflows, getFilteredItems]
  );
  const filteredSnippets = useMemo(
    () => getFilteredItems(snippets, 'snippet'),
    [snippets, getFilteredItems]
  );
  ...
  // Progressive loading voor performance optimization - commented out for now
  ...
  const favoritesReorderHandler = useCallback(
    (reorderedItems) => {
  ...
  const workflowsReorderHandler = useCallback(
    (reorderedItems) => {
  ...
  const templatesReorderHandler = useCallback(
    (reorderedItems) => {
  ...
  const snippetsReorderHandler = useCallback(
    (reorderedItems) => {
  ...
  const sections = [
    ...
    {
      type: 'workflows',
      data: workflowsPagination.currentItems,
      pagination: workflowsPagination,
      fullData: filteredWorkflows,
    },
    {
      type: 'templates',
      data: templatesPagination.currentItems,
      pagination: templatesPagination,
      fullData: filteredTemplates,
    },
    {
      type: 'snippets',
      data: snippetsPagination.currentItems,
      pagination: snippetsPagination,
      fullData: filteredSnippets,
    },
  ];
  ...
  */
  ...
  useEffect(() => {
    ...
    if (previousLength !== allItems.length && previousLength > 0) {
      ...
      if (allItems.length > previousLength) {
        ...
        if (
          !focusState.expandedSections?.has('favorites') &&
          visibleSections.has('favorites') &&
          favoritesVisibility.isVisible
        ) {
          newlyExpandedSection = 'favorites';
        } else if (
          !focusState.expandedSections?.has('workflows') &&
          visibleSections.has('workflows') &&
          workflowsVisibility.isVisible
        ) {
          newlyExpandedSection = 'workflows';
        } else if (
          !focusState.expandedSections?.has('templates') &&
          visibleSections.has('templates') &&
          templatesVisibility.isVisible
        ) {
          newlyExpandedSection = 'templates';
        } else if (
          !focusState.expandedSections?.has('snippets') &&
          visibleSections.has('snippets') &&
          snippetsVisibility.isVisible
        ) {
      ...
      const itemStillExists =
        focusState.itemId && findItemIndexInSection(focusState.itemId, focusState.section) >= 0;
  ...
  const findBestFocusAlternative = (
    newItems,
    previousIndex,
    previousItems,
    ...
    const sectionStillExists = Array.from(sectionIndexMap.values()).some(
      (entry) => entry.section === previousSection
    );
    ...
    if (!sectionStillExists && previousSection) {
      ...
      newItems.forEach((item, index) => {
        ...
        if (itemSectionIndex > removedSectionIndex) {
          ...
          if (
            bestSectionIndex === -1 ||
            itemSectionIndex < bestSectionIndex ||
    ...
    if (targetItem && previousSection) {
      ...
      newItems.forEach((item, idx) => {
        if (item.id === targetItem.id) {
          const itemSection = sectionIndexMap.get(idx)?.section;
          allTargetMatches.push({ index: idx, section: itemSection });
        }
      });
      ...
      if (allTargetMatches.length > 1) {
        ...
        const nonFavoritesMatches = allTargetMatches.filter(
          (match) => match.section !== 'favorites'
        );
                        ...
                        keyboardNavigation={{
                          ...
                          getFocusProps: () => {
                            ...
                            const isKeyboardFocused =
                              keyboardNavigation.isActive &&
                              keyboardNavigation.focusedIndex === globalIndex;
          ...
          return (
            <CollapsibleSection
              ...
              onCreateNew={() =>
                onEditWorkflow({
                  folderIds:
                    selectedFolderId && selectedFolderId !== 'root' ? [selectedFolderId] : [],
                })
              }
              actionButton={
                <button
  ...
  return (
    <div className="flex h-screen bg-secondary-50 dark:bg-secondary-900">
      ...
      {/* Todo Sidebar */}
      <CollapsibleTodoSidebar currentFolderId={selectedFolderId} />
