...
const ResizableSection = ({
  ...
  // Use useRef to store handlers to avoid circular dependencies
  const handlersRef = useRef({});

  // Mouse move handler for resize
  handlersRef.current.handleMouseMove = useCallback(
    (e) => {
  ...
  handlersRef.current.handleMouseUp = useCallback(() => {
    if (!shouldBeResizable) return;
  ...
  return (
    <div
      className={`resizable-section section-boundary-restricted ${dynamicClassName}`}
      {...restProps}
    >
      {/* Section Header */}
      <div
        className={`w-full flex items-center justify-between overflow-hidden transition-colors duration-200 group border ${
          (count || itemCount || 0) > 0
      ...
      >
        <button
          ...
          className={`flex-1 flex items-center justify-between p-3 focus:outline-none ${
            (count || itemCount || 0) === 0 && !onCreateNew && !alwaysCollapsible
              ? 'cursor-default'
              : 'cursor-pointer'
          }`}
        ...
        >
          <div className="flex items-center space-x-2">
            {hideChevronWhenEmpty &&
            (hasContent !== null ? !hasContent : totalCount === 0) &&
            !isVisible ? (
        ...
        <div className="overflow-hidden">
          <div
            ...
            className={`transition-all duration-300 ease-in-out ${
              (hasContent !== null ? hasContent : hasContentLocal) &&
              !className.includes('no-content-padding')
                ? 'pt-4'
                : ''
            } relative`}
            style={{
          ...
          >
            <div className={shouldBeResizable ? 'pr-2' : ''}>{children}</div>
