    ...
    <>
      ...
      >
        ...
        <div className="flex items-center gap-3 min-h-[4rem]">
          {/* Drag Handle */}
          <GripVertical
            className={`w-5 h-5 flex-shrink-0 ${isClassicDark() ? 'text-secondary-400' : 'text-secondary-600 dark:text-secondary-400'}`}
          />

          <div className="flex-1 pb-4">
            <h3
              className={`font-semibold mb-2 ${isClassicDark() ? 'text-secondary-100' : 'text-secondary-900 dark:text-secondary-100'}`}
            >
              {item.name}
            </h3>
            <p
              className={`text-sm line-clamp-2 ${isClassicDark() ? 'text-secondary-300' : 'text-secondary-600 dark:text-secondary-300'}`}
            >
